...
from (/*Grouping and counting the patients*/
  ...
  FROM
    (/*Getting the age group with the obs id*/
      SELECT
        CASE WHEN datediff(now(), p.birthdate) BETWEEN 5114 AND 6940
          THEN '15-19 Yrs'
        WHEN datediff(now(), p.birthdate) BETWEEN 6941 AND 8766
          THEN '20-24 Yrs'
        WHEN datediff(now(), p.birthdate) BETWEEN 8767 AND 17897
      ...
      WHERE cn.name = 'Immediate post partum record'
            ...
            AND datediff(now(), p.birthdate) < 17898/*Age is less than 49 years*/
